...
#$Q_file=<STDIN>;
$Q_file="/data2/Go_1PHT/folding_traj/Go_1PHT_136/Concat_136.xtc.CA.Q";
chomp ($Q_file);
...
#	if ($diff==1)				# diff==1 for the folding transitions
	if ($diff==-1)				# diff==-1 for the unfolding transitions		
        {
        $UOUT[$a]=$UDATA;
	$FOUT[$a]=$FDATA;
#	print "$UOUT[$a] $FOUT[$a]\n";	
...
$Qcount=$Qcount+$Qline;
$line_no++;
	if ((defined ($UOUT[$c])) && (defined ($FOUT[$c])) && ($line_no<=$UOUT[$c]) && ($line_no>=$FOUT[$c])) # The signs are opposite 
	{
	$QOUT[$d]=$Qline;
	$FRAMES[$d]=$line_no;
	$d++;
	}	
	if ((defined ($UOUT[$c])) && ($line_no>$UOUT[$c]))
	{
...
{
`echo 1 | trjconv -f ../Concat_136.xtc -s ../Output.tpr -fr frames_$g.ndx -o frames_$g.xtc`;
}
